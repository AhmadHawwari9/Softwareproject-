...
const sendMessageToDb = async (sender_id, receiver_id, message, img_id) => {
    const sql = `
        INSERT INTO messages (sender_id, receiver_id, message, img_id, timestamp)
        VALUES (?, ?, ?, ?, NOW())
    `;
    return new Promise((resolve, reject) => {
        db.query(sql, [sender_id, receiver_id, message, img_id], (err, results) => {
            if (err) {
                console.error("Error inserting message:", err); // Add logging
                return reject(err);
            }
            resolve({ id: results.insertId, sender_id, receiver_id, message, img_id });
        });
    });
};
