...
const saveDoctorAvailability = (doctorId, days, startTime, endTime, appointmentDuration) => {
  return new Promise((resolve, reject) => {
    ...
    db.query(selectQuery, [doctorId], (err, results) => {
      ...
      function insertAvailability() {
        const insertQuery = `
          INSERT INTO doctor_availability (doctor_id, days, start_time, end_time, appointment_duration)
          VALUES (?, ?, ?, ?, ?)
        `;

        db.query(insertQuery, [doctorId, days, startTime, endTime, appointmentDuration], (err, insertResults) => {
          if (err) {
            reject(err);
            return;
          }
...
const getDoctorAvailability = (doctorId) => {
  return new Promise((resolve, reject) => {
    const query = `
      SELECT days, start_time, end_time, appointment_duration
