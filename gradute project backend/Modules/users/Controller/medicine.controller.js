  ...
  try {
    ...
    for (const { doctor_id, timing, medicine_name } of medicationsToNotify) {
      ...
      if (!notificationSent) {
        ...
        INSERT IGNORE INTO notifications (Sender_id, reciver_id, typeofnotifications, is_read, medication_time)
        VALUES (?, ?, ?, 0, ?)
        `;
        await db.query(notificationQuery, [doctor_id, receiverId, message, timing]);
