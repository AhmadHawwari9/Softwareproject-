...
import 'Gps.dart'; // Add this import
        ...
        builder: (context, snapshot) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              if (notificationType.startsWith('Emergency Alert:')) {
                      ...
                      return Padding(
                        ...
                        child: Card(
                          ...
                          child: ListTile(
                            ...
                            subtitle: Text(
                              'Emergency Alert: Press to see the position',
                              style: TextStyle(
                                fontWeight: FontWeight.bold,
                                fontSize: 16,
                                color: isUnread ? Colors.black : Colors.grey,
                            ...
                            onTap: () {
                              try {
                                ...
                                print('Full notification: ${notification.toString()}');

                                // Use the `typeofnotifications` field instead of `message`
                                String notificationType = notification['typeofnotifications'] ?? '';

                                // Check if the notification type is not empty
                                if (notificationType.isEmpty) {
                                  print('Error: Notification type is empty');
                                  return;
                                }

                                // Check if the notification type contains the expected format
                                if (notificationType.startsWith('Emergency Alert:') && notificationType.contains('Lat=') && notificationType.contains('Lng=')) {
                                  // Extract latitude and longitude from the notification type
                                  String latLng = notificationType.split('Emergency Alert: ')[1];
                                  List<String> parts = latLng.split(', ');
                                  if (parts.length == 2) {
                                    double latitude = double.parse(parts[0].split('=')[1]);
                                    double longitude = double.parse(parts[1].split('=')[1]);

                                    // Navigate to GPSMapPage with the extracted coordinates
                                    ...
                                    print('Invalid latitude/longitude format in notification type: $notificationType');
                                  }
                                } else {
                                  print('Invalid emergency alert notification type format: $notificationType');
                                }
                              } catch (e) {
                                print('Error parsing emergency alert notification type: $e');
                              }
                            },
                          ),
                        ),
