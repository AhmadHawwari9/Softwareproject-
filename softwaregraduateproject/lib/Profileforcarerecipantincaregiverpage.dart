...
class UserDetailsPageforcarerecipantincaregiverpage extends StatefulWidget {
  final String id;
  final String savedToken;

  const UserDetailsPageforcarerecipantincaregiverpage({Key? key, required this.id, required this.savedToken}) : super(key: key);

  @override
  _UserDetailsPageState createState() => _UserDetailsPageState();
}

class _UserDetailsPageState extends State<UserDetailsPageforcarerecipantincaregiverpage> {
  late Map<String, dynamic> userData;
  bool isLoading = true;
  bool isError = false;
  bool isRequested = false; // Tracks if the user has already requested
  bool isFollowing = true; // Track if the user is already following
  ...
  Future<void> _toggleFollowRequest() async {
    ...
    final endpoints = {
      'getUnfollowNotifications': Uri.parse('http://10.0.2.2:3001/getUnfollowNotifications'),
      'unfollow': Uri.parse('http://10.0.2.2:3001/unfollow'),
      'deleteUnfollowRequest': Uri.parse('http://10.0.2.2:3001/DeleteUnfollowRequest/${widget.id}'),
    };
    ...
    try {
      ...
      if (hasUnfollowRequest) {
        // Unfollow request already exists, delete it
        ...
        print('Delete Unfollow Request Response: ${response.statusCode}, ${response.body}');
        if (response.statusCode == 200) {
          setState(() {
            hasUnfollowRequest = false;
            isFollowing = true; // Revert to following
          });
        }
      } else if (isFollowing) {
        // Send Unfollow request (if Following)
        setState(() {
          isFollowing = false; // Change to Unfollow Requested state
          hasUnfollowRequest = true; // Set the flag to show "Unfollow Requested"
        });

        // Make the unfollow request to the server
        ...
        // Follow request if not following
        setState(() {
          isRequested = true; // Change to requested state
          isFollowing = false; // Stop following
        });

        // Make the follow request to the server
        response = await http.post(
          endpoints['unfollow']!,
          headers: {
            'Authorization': 'Bearer ${widget.savedToken}',
        ...
        if (response.statusCode == 200) {
          setState(() {
            isRequested = true; // Follow request sent
            isFollowing = false; // Not following anymore
          });
        } else {
          final errorData = json.decode(response.body);
          ScaffoldMessenger.of(context).showSnackBar(
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    _buildActionButton(
                      context,
                      label: hasUnfollowRequest ? 'Unfollow Requested' : 'Following',
                      icon: Icons.check, // Same icon for both states
                      color: Colors.teal, // Same color for consistency
                      onPressed: () async {
                        await _toggleFollowRequest(); // Perform the toggle action
